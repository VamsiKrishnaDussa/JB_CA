...
module.exports = function inboundCallActivity(app, options) {
    ...
    app.get('/activity/app/', (req, res) => res.redirect('/'));

    // Serve Config JSON
    app.get('/activity/config/config-json.js', (req, res) => {
        res.type('application/javascript');
        res.send(`module.exports = ${JSON.stringify(configJSON())};`);
    });
    ...
    async function authenticate() {
        try {
            ...
            console.error("❌ SFMC Authentication Failed:", error.response ? error.response.data : error.message);
...
//     // Serve Config JSON
//     app.get('/activity/config/config-json.js', (req, res) => res.status(200).json(configJSON(req)));
