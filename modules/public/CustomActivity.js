...
//                 console.log("printing reponse function");
...
//                 // Check if the response contains optInStatus
//                 if (!response.optInStatus) {
//                     console.error("optInStatus is missing.");
//                     alert("Missing optInStatus in API response.");
//                     return;
//                 }
...
//                 payload.arguments.execute.outArguments = [{ OptInStatus: response.optInStatus }];
//                 payload.outcome = branchResult;
//                 console.log("Payload updated:", JSON.stringify(payload, null, 2));
...
define(["postmonger"], function (Postmonger) {
    ...
    function onInitActivity(data) {
        ...
        let phoneNumber = "";
        if (payload.arguments.execute.inArguments.length > 0) {
            phoneNumber = payload.arguments.execute.inArguments[0].phoneNumber || "";
        }

        if (phoneNumber) {
            console.log("Phone Number found:", phoneNumber);
            makeAPICall(phoneNumber);
        } else {
            console.warn("No phone number found in inArguments. Activity will not make API call.");
        }
    ...
    function makeAPICall(phoneNumber) {
        console.log("Making API Call for phone number:", phoneNumber);
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (!response.optInStatus) {
                    console.error("optInStatus is missing in API response.");
                    return;
                }
